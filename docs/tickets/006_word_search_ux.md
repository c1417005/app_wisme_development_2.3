...
**PC（768px 以上）:**
- メモ入力エリア右横に固定幅（例: `w-80`）のサイドバーとして常駐。
- ページ全体を2カラムレイアウト（`grid grid-cols-[1fr_20rem]`）にする。

**スマホ（767px 以下）:**
- 画面下部に折りたたんだパネル（高さ `h-12`）として配置。
- スワイプアップ（または「検索」ボタンタップ）で高さを `h-64` 等に展開する。
- Vue.js の `data` で展開状態を管理し、`transition` で滑らかに切り替える。

### 2-3. スケルトン UI の実装（フロントエンド）
...
| `templates/wisme/page_create.html` | サイドバーレイアウト・スケルトン UI・自動紐付けインジケーターを追加 |
...
- [ ] 検索成功時に自動紐付けインジケーター（緑チェック + テキスト）が3秒表示される
- [ ] 2 回目以降に同じ単語を検索すると DB から即座に返る（API を呼ばない）
- [ ] ネットワークエラー時にエラーメッセージが表示される
- [ ] PC 表示でサイドバーがメモ入力エリア横に常駐する
- [ ] スマホ表示で下部パネルが展開・折りたたみできる
- [ ] スマートフォン表示でもスケルトン UI が崩れない

### パフォーマンス確認
- [x] `SearchedWord.word` に `db_index=True` が設定されている（`unique=True` はマルチユーザー設計と相容れないため不採用）

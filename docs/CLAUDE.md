...
- **Page** — 読書メモ。UUID PK、タイトル、**本全体の感想（`thoughts`）**、日付、任意の画像、`image_url`（Google Books API から取得したサムネイル URL）を持つ。`delete()` をオーバーライドし、削除時に画像ファイルをディスクから削除。
...
### 書籍サムネイル自動取得（ticket 013）
- Google Books API を使い、タイトル＋著者名から表紙画像候補（最大5件）を取得してユーザーが選択する。
- `Page.image_url`（URLField）に選択した URL を保存。画像ファイルはサーバーに保存しない。
- JS (Fetch API) で非同期取得。`http://` URL は `https://` に置換して扱う。
...
| チケット | タイトル | 実装 | 自動テスト |
|---|---|---|---|
| 001 | カスタムユーザーモデル | 完了 | 未作成 |
| 002 | ユーザー認証機能 | 完了 | 完了（12件） |
| 003 | ユーザープロフィール管理機能 | 完了 | 完了（6件） |
| 004 | 単語帳機能 | 完了 | 完了（5件） |
| 005 | クイズ機能（フラッシュカード形式）| 完了 | 完了（6件） |
| 006 | 単語検索 UX 改善（サイドバー・スケルトン UI） | 完了 | 完了（6件） |
...
> **実装完了後の必須手順：** `python manage.py test` を実行し、通過した項目に対応するチケットファイル（`docs/tickets/XXX_*.md`）の DoD チェックボックスを `[x]` に更新すること。自動テストでは確認できない項目（手動確認が必要なもの）はチェックしない。
